...
Compilation:
 ...
 - cd DFPT  "make" to produce normal and SC-field DF 1-st order perturbation executable in ithe "bin"   
...
The "Hub4x4" and "Hub4x4SC" folderis contain  outputs for normal and SC-state. 

Calculations:
 - First run.sh0 "build_ref" (sequential) DQMC for the Reference Green's function (use larger number of QMC-sweeps)
 - Then  run.sh1 the FTPT in normal state (dfpt_mpi) or SC-state (dfsc_mpi) with maximum possible MPI processors and QMC-sweeps
