...
contains
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  DF-QMC  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
  ...
  subroutine DFPT_Meas_Dual_Sigma(T1, tau, mk, ref_GF_full, dual_G0_full, GF_up_full, GF_dn_full, Sdual)
    ! 
    ! Purpose
    ! =======
    !    
    ...
    complex(wp) :: Xi, zf, Hc, Sk, Sd, Eq(T1%properties(1)%n*tau%L)
  ...
  subroutine DFPT_Read_Pert_Params(tp,mu)
    real(wp), intent(inout) :: tp, mu
  ...
  subroutine DFPT_build_dual_G0(tp,mu,N,L,k_vecs,ref_GF_kkp_tau,dual_G0,dual_G0_full,Nom,dtau)
    ! 
    ! Builds the "bare" dual GF from reference G from previous run
    ! and with the "perturbation matrix", for a given t' and mu
    ! 
    
    ! Input
    integer,  intent(in) :: N, L, Nom
    real(wp), intent(in) :: tp, mu, dtau
    ...
    complex(wp), dimension(N,0:Nom) :: gkw0, Gkwdf
    real(wp), dimension(N) :: tdk
    ...
    do kk = 1, N
        ! Perturbation (mu and t'-dispersion) 
        tdk(kk) = - 4.0_wp * tp * cos(k_vecs(kk,1)) * cos(k_vecs(kk,2)) - mu
        do iom = 0, Nom
            Gkwdf(kk,iom) = tdk(kk) / ( ONE - gkw0(kk,iom) * tdk(kk) )
  ...
  subroutine DFPT_Get_Lattice_G(tp,mu,N,L,k_vecs,ref_GF_kkp_tau,Sdual,Nom,dtau,lattG)
    ! 
  ...
  subroutine DFPT_Write_Lattice_G(N,L,dtau,lattG,Sdual)
    ! 
    ...
    do i = 1, Nx
      do j = 1, Nx
       ...
       !write(12,*)"#Kx Ky", (i-1)/pi/real(N), (j-1)/pi/real(N)
        ...
        write(12,fmtstr) wn, lattG(ijt),Sdual(ijt)
        end do
        write(12,*)
      end do  
    end do
